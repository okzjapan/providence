...
class KeibaRaceEntry(Base):
    ...
    # JRDB prediction indices (展開予想系, pre-race)
    ten_index_pred: Mapped[float | None] = mapped_column(Float)
    pace_index_pred: Mapped[float | None] = mapped_column(Float)
    agari_index_pred: Mapped[float | None] = mapped_column(Float)
    position_index_pred: Mapped[float | None] = mapped_column(Float)
    pace_prediction: Mapped[str | None] = mapped_column(String(1))
    # JRDB evaluation indices (pre-race)
    improvement_grade: Mapped[int | None] = mapped_column(Integer)
    upset_index: Mapped[int | None] = mapped_column(Integer)
    longshot_index: Mapped[int | None] = mapped_column(Integer)
    heavy_aptitude_code: Mapped[int | None] = mapped_column(Integer)
    hoof_code: Mapped[str | None] = mapped_column(String(2))
    blinkers: Mapped[str | None] = mapped_column(String(1))
    turf_aptitude_code: Mapped[str | None] = mapped_column(String(1))
    dirt_aptitude_code: Mapped[str | None] = mapped_column(String(1))
    start_index: Mapped[float | None] = mapped_column(Float)
    delay_rate: Mapped[float | None] = mapped_column(Float)
    stable_rank: Mapped[int | None] = mapped_column(Integer)
...
class KeibaRaceResult(Base):
    ...
    jrdb_idm: Mapped[int | None] = mapped_column(Integer)
    base_score: Mapped[int | None] = mapped_column(Integer)
    track_bias: Mapped[int | None] = mapped_column(Integer)
    pace_factor: Mapped[int | None] = mapped_column(Integer)
    late_start_correction: Mapped[int | None] = mapped_column(Integer)
    positioning_correction: Mapped[int | None] = mapped_column(Integer)
    disadvantage_correction: Mapped[int | None] = mapped_column(Integer)
    course_position: Mapped[int | None] = mapped_column(Integer)
    ten_index: Mapped[float | None] = mapped_column(Float)
    agari_index: Mapped[float | None] = mapped_column(Float)
    pace_index: Mapped[float | None] = mapped_column(Float)
